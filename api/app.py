    ...
    try:
        ...
        for result in rv:
            content = {
                ...
                'fecha_reserva': result[5].strftime('%Y-%m-%d') if isinstance(result[5], datetime.date) else "No especificado",
                'hora_reserva': result[6].strftime('%H:%M:%S') if isinstance(result[6], datetime.time) else "No especificado",
                'sede': result[7] or "No especificado"
            }
            payload.append(content)
        print("Datos a enviar:", payload)  # Agrega esta línea antes de la llamada a jsonify
        return jsonify(payload)
    except Exception as e:
...
def registrar_recomendacion_ps():
    try:
        ...
        check_psicologo_sql = "SELECT COUNT(*) FROM profesor WHERE identificacion = %s"
        cur.execute(check_profesor_sql, (id_profesor,))
        profesor_count = cur.fetchone()[0]
        print(f"Resultados de búsqueda de profesores: {psicologo_count}")
        if psicologo_count == 0:
            cur.close()
            return jsonify({'message': 'El profesor especificado no existe en la base de datos'}), 400
        ...
        VALUES (%s, %s, %s, %s)
        """
        values = (id_profesor, id_estudiante, recomendacion, fecha_recomendacion)
...
def enfermeria():
    ...
    if 'role' in session and session['role'] == 'enfermera':
        return render_template('html/abono.html')
    else:
...
def psicologos():
    return render_template('html/profesores.html')
